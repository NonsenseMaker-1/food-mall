window.SHOP = {
  "name": "[가게 이름]",
  "tagline": "[한 줄 소개]",
  "hoursWeek": "[평일 운영시간]",
  "hoursWeekend": "[주말 운영시간]",
  "lunch": "[점심시간]",
  "phone": "[대표 전화번호]",
  "email": "[대표 이메일]"
};

window.COMPANY = {
  "name": "[상호명]",
  "ceo": "[대표자명]",
  "bizNo": "[사업자등록번호]",
  "mailOrderNo": "[통신판매업신고번호]",
  "address": "[사업장 주소]",
  "privacyOfficer": "[개인정보관리책임자]",
  "founded": "[설립연도]",
  "factory": "[생산지/공장 주소]"
};

window.ABOUT = {
  "slogan": "[슬로건 — 예: 정직한 식탁을 만듭니다]",
  "lead": "[메인 소개 문구 — 회사와 제품을 한 문장으로]",
  "greeting": "[대표 인사말. 회사를 시작하게 된 이유, 품질에 대한 약속 등을 이 자리에 넣습니다.]",
  "story": "[회사 소개 본문. 어떤 식품을 다루는지, 누구에게 공급하는지 적습니다.]",
  "vision": "[경영 이념 / 비전]",
  "values": [
    { "t": "[가치 01]", "d": "[설명]" },
    { "t": "[가치 02]", "d": "[설명]" },
    { "t": "[가치 03]", "d": "[설명]" }
  ],
  "facts": [
    { "n": "[숫자 01]", "l": "[항목 — 예: 거래처]" },
    { "n": "[숫자 02]", "l": "[항목 — 예: 취급 품목]" },
    { "n": "[숫자 03]", "l": "[항목 — 예: 공급 경력]" }
  ]
};

window.CATEGORIES = [
  { "id": "beef", "name": "[수입소]", "image": "img/cat-beef.jpg" },
  { "id": "ipork", "name": "[수입돼지]", "image": "img/cat-pork.jpg" },
  { "id": "chicken", "name": "[닭/오리]", "image": "img/cat-chicken.jpg" },
  { "id": "seafood", "name": "[수산물]", "image": "img/cat-seafood.jpg" },
  { "id": "grocery", "name": "[식자재]", "image": "img/cat-grocery.jpg" },
  { "id": "gift", "name": "[선물세트]", "image": "img/cat-gift.jpg" }
];

window.PRODUCTS = [
  { "id": "p02", "name": "[제품명 01]", "spec": "[규격]", "category": "beef", "categoryName": "[수입소]", "price": "[판매가]", "intro": "[제품 소개 한 줄]", "origin": "[원산지]", "volume": "[내용량]", "expiry": "[유통기한]", "storage": "[보관방법]", "ingredients": "[원재료명]", "image": "" },
  { "id": "p04", "name": "[제품명 02]", "spec": "[규격]", "category": "ipork", "categoryName": "[수입돼지]", "price": "[판매가]", "intro": "[제품 소개 한 줄]", "origin": "[원산지]", "volume": "[내용량]", "expiry": "[유통기한]", "storage": "[보관방법]", "ingredients": "[원재료명]", "image": "" },
  { "id": "p05", "name": "[제품명 03]", "spec": "[규격]", "category": "chicken", "categoryName": "[닭/오리]", "price": "[판매가]", "intro": "[제품 소개 한 줄]", "origin": "[원산지]", "volume": "[내용량]", "expiry": "[유통기한]", "storage": "[보관방법]", "ingredients": "[원재료명]", "image": "" },
  { "id": "p06", "name": "[제품명 04]", "spec": "[규격]", "category": "seafood", "categoryName": "[수산물]", "price": "[판매가]", "intro": "[제품 소개 한 줄]", "origin": "[원산지]", "volume": "[내용량]", "expiry": "[유통기한]", "storage": "[보관방법]", "ingredients": "[원재료명]", "image": "" },
  { "id": "p07", "name": "[제품명 05]", "spec": "[규격]", "category": "grocery", "categoryName": "[식자재]", "price": "[판매가]", "intro": "[제품 소개 한 줄]", "origin": "[원산지]", "volume": "[내용량]", "expiry": "[유통기한]", "storage": "[보관방법]", "ingredients": "[원재료명]", "image": "" },
  { "id": "p08", "name": "[제품명 06]", "spec": "[규격]", "category": "gift", "categoryName": "[선물세트]", "price": "[판매가]", "intro": "[제품 소개 한 줄]", "origin": "[원산지]", "volume": "[내용량]", "expiry": "[유통기한]", "storage": "[보관방법]", "ingredients": "[원재료명]", "image": "" }
];

window.IMAGES = {
  "hero": "",
  "company": "",
  "about": "",
  "notice": "",
  "products": "",
  "productPage": "",
  "contact": ""
};

window.IMAGE_POS = {
  "hero": "center center",
  "company": "center center",
  "about": "center center",
  "notice": "center center",
  "products": "center center",
  "productPage": "center center",
  "contact": "center center"
};
